...
class test_innoKey(unittest.TestCase):
    def setUp(self):
    	getData()
...
#        print dt
    ...
    def test_corpNum(self):
        ...
        #公司结果数
        try:
            compNum=repdt["data"]["company"]["count"]
        except Exception, e:
            self.assertEqual (1,0,u"接口未返回结果")
        self.assertGreaterEqual(int(compNum),6000,u'官网公司搜索关键字-金融,数量少于预期值6000(before 6601),本次结果为: '+str(compNum)+" "+recontent.url)
