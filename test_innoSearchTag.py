...
class test_innoTag(unittest.TestCase):
    def setUp(self):
    	getData()
...
#        print dt
    ...
    def test_corpNum(self):
        ...
        #公司结果数
        try:
            compNum=repdt["data"]["tags"]["count"]
        except Exception, e:
            self.assertEqual (1,0,u"接口未返回结果")
        self.assertGreaterEqual(int(compNum),4300,u'官网公司搜索标签-金融,数量少于预期值4300(before 4733),本次结果为: '+str(compNum)+" "+recontent.url)
