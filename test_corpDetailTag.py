...
#        print dt
    ...
    def test_tag(self):
        ...
        try:
            compTag=repdt["data"][0]["tags"]
            compName=repdt["data"][0]["tagName"]
        except Exception, e:
            self.assertEqual (1,0,u"接口未返回结果")
        self.assertGreaterEqual(len(compTag),50,u'公司详情标签列表接口,返回的标签列表不全,返回的标签列表个数预期为69个,现在个数及标签列表为: '+str(len(compTag))+"  "+str(','.join(compTag))+" "+recontent.url)
